...
replicateA ::
  ...
  -> f (List a)
--replicateA n fa = fa
replicateA 1 fa = (:. Nil) <$> fa
replicateA n _ | n <= 0 = pure Nil
...
filtering f (a :. rest) = (\r acc -> if r then a :. acc else acc) <$> f a <*> filtering f rest
