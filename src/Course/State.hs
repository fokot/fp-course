...
instance Functor (State s) where
  (<$>) ::
    ...
    -> State s a
    -> State s b
  (<$>) f s = State ( \ss -> let (a, b) = (runState s ss) in (f(a), b) )
...
instance Applicative (State s) where
  ...
  (<*>) fs s =
    State (\i ->
      let (f, newState) = runState fs i
          (a, newerState) = runState s newState
      in  (f a, newerState)
    )
...
instance Monad (State s) where
  ...
  (=<<) f s =
    State(\i ->
      let (a, newState) = (runState s i)
          newRunState = runState $ f a
...
findM f (a :. rest) =
  f a >>= \res -> if res then pure (Full a) else findM f rest
